...
function EventSource(Sender) {
  return class extends Sender {
    constructor(...args) {
      super(...args), _defineProperty(this, "_eventHandlers", {
        click: {}
      });
    }
...
const getChartOptions = options => {
  ...
  const sharedEmbedOptions = getSharedEmbedOptions(options), {chartId: chartId, refreshInterval: refreshInterval} = options;
...
class Chart extends(CommonChart(Filterable(Refreshable(EventSource(ChartEventSender))))){
  async render(container) {
    await super.render(container), await this._send("ready");
  }
...
class Dashboard extends(Filterable(Refreshable(DashboardEventSender))){
  constructor(...args) {
    super(...args), _defineProperty(this, "charts", {});
  }
  async render(container) {
